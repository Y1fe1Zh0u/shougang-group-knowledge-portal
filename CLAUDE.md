...
> ⚠️ **后端 rsync 必须排除 `app/config/data/`**，否则会用本地的 `portal_config.json` / `bisheng_runtime.json` 覆盖生产 admin UI 攒下的全部配置（业务域、空间绑定、QA、apps、BiSheng 接入凭证全没）。本地这两份是开发态，跟生产 schema 一致但内容八杆子打不着。
>
> **后端不要加 `--delete`** —— 否则生产上 BFF 写出的 `*.bak.*` 兜底备份与首次启动 seed 的 `data/` 目录都会被清掉。
...
# 后端（重启 BFF）—— 注意 exclude 列表 + 先 dry-run 看清要传什么
rsync -avzn --exclude='.venv' --exclude='.env' --exclude='__pycache__' \
  --exclude='.pytest_cache' --exclude='app/config/data/' \
  backend/ root@192.168.106.114:/opt/shougang-portal/        # 加 -n 是 dry-run
...
`/opt/shougang-portal/app/config/data/` 里如果发现 `portal_config.json.bak.<timestamp>`，那是历史上人手工建的兜底（**不是 BFF 自动备份**——代码里没这个机制）。要恢复就 `cp .bak.<ts> portal_config.json && systemctl restart shougang-portal`。

如果 admin 配置被错误覆盖且没有 .bak：
1. 立刻 `ssh root@192.168.106.114 'cp /opt/shougang-portal/app/config/data/portal_config.json /opt/shougang-portal/app/config/data/portal_config.json.bak.before-fix-$(date +%s)'` 先冻一份现状
2. 删 `portal_config.json` + 重启 BFF → 会 seed 出 `DEFAULT_PORTAL_CONFIG`（14 个默认业务域、6 个默认 apps、spaces 为空），然后从 admin UI 重新绑空间
...
2. **`portal_config.json` / `bisheng_runtime.json` 是 admin UI 的产物，不要手改文件后 commit，也不要 rsync 覆盖** —— 先用 `/admin` UI 改，再观察 JSON 变化，再决定是否 commit。手改容易跟 admin UI 的下次保存竞态。**部署时务必排除整个 `app/config/data/` 目录**：本地的开发态会瞬间覆盖生产 admin 攒下的全部业务域 / 空间绑定 / QA / apps / BiSheng 凭证，即使 schema 一致也会瞎掉。详见 §7 的 rsync 命令与回滚章节。
